...
export const content: ToolLocaleContent<FermentationUI> = {
  ...
  seo: [
    ...
    { type: 'paragraph', html: 'Во время ферментации используйте температуру и объём как практические контрольные точки. Временной диапазон является оценкой: наблюдайте за структурой теста и подтверждайте окончание брожения пива стабильным измерением.' },
  ],
