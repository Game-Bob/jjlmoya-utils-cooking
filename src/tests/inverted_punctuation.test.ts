...
function findLastColonBoundary(beforeMark: string): number {
  const lastColon = beforeMark.lastIndexOf(':');
  if (lastColon !== -1) {
    if (lastColon === beforeMark.length - 1 || /\s/.test(beforeMark[lastColon + 1])) {
      return lastColon;
    }
  }
  return -1;
...
function findLastDotBoundary(beforeMark: string): number {
  const lastDot = beforeMark.lastIndexOf('.');
  if (lastDot === -1) return -1;
  if (lastDot === beforeMark.length - 1 || /\s/.test(beforeMark[lastDot + 1])) {
    return lastDot;
  }
  for (let i = lastDot - 1; i >= 0; i--) {
    if (beforeMark[i] === '.' && /\s/.test(beforeMark[i + 1])) {
      return i;
    }
  }
  return -1;
...
function sentenceStart(text: string, endIndex: number): string {
  ...
  const boundary = Math.max(
    findLastDotBoundary(beforeMark),
    findLastColonBoundary(beforeMark),
